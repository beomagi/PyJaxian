...
filelist=glob.glob(r'{}*'.format(pagefolder)) #globally permitted file list
scriptlist=glob.glob(r'{}**'.format(scriptfolder),recursive=True) #permitted scripts
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if location.startswith("/script|"):
            ...
            if scriptfolder+params[0] in scriptlist:
                cmd2run = scriptfolder+(" ".join(params))
                try:
                ...
                print(scriptfolder+params[0]+" is not in scriptlist")
